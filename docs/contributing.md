...
There are two version numbers, on purpose:

- **App version** (`package.json` → `APP_VERSION`, shown in the header) —
  bump it and tag `vX.Y.Z` for a release. `main` deploys automatically.
- **Generator version** (`GENERATOR_VERSION` in
  `src/engine/generator/generator.ts`) — bump it whenever generation output
  for a seed can change (rules, tables, weights, materialisers). Generated and
  shared styles record it, so an old share link can be identified as belonging
  to an earlier algorithm instead of silently producing a different result.
  After a bump run `npm run engine:data` and update the Python engine until
  `npm run engine:test` passes.

Release steps: update `package.json` version → `git tag vX.Y.Z` →
